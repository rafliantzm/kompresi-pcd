...
export const BENCHMARK_TIMING_MODE = "REPEATED_RUN_BENCHMARK";
...
export const RESOLUTION_EXPERIMENT_LEVELS = [128, 64, 32, 16, 8];
export const RESOLUTION_EXPERIMENT_CONTENT_IDS = ["logo", "jalan", "pola"];
export const RESOLUTION_EXPERIMENT_RESOLUTIONS = [
  [640, 360],
  [1280, 720],
  [1920, 1080],
  [3840, 2160],
...
export const RESOLUTION_EXPERIMENT_RAW_COLUMNS = [
  "experiment_id",
  "content_id",
  "content_category",
  "source_format",
  ...
  "rle_total_max_ms",
  "rle_total_std_ms",
  "huffman_encode_mean_ms",
  "huffman_decode_mean_ms",
  "huffman_total_mean_ms",
  "huffman_total_min_ms",
  "huffman_total_max_ms",
  "huffman_total_std_ms",
  "rle_byte_identical",
  "huffman_byte_identical",
  "rle_payload_bits_per_pixel",
  "huffman_payload_bits_per_pixel",
  "rle_pairs_per_1000_pixels",
  "rle_time_ms_per_megapixel",
  "huffman_time_ms_per_megapixel",
];

export const SUMMARY_BY_RESOLUTION_COLUMNS = [
  "width",
  "height",
  "pixel_count",
  "row_count",
  ...
  "mean_rle_total_time_ms",
  "mean_huffman_total_time_ms",
  "mean_rle_time_ms_per_megapixel",
  "mean_huffman_time_ms_per_megapixel",
];
...
export function serializeResolutionExperimentRawCsv(rows) {
  return serializeCsvRows(rows, RESOLUTION_EXPERIMENT_RAW_COLUMNS);
}

export function serializeSummaryByResolutionCsv(rows) {
  return serializeCsvRows(rows, SUMMARY_BY_RESOLUTION_COLUMNS);
}
...
export function buildResolutionExperimentRows(rows, experimentId = "CONTROLLED_RESOLUTION_EXPERIMENT") {
  return rows
    .filter((row) => isControlledResolutionExperimentRow(row))
    .map((row) => {
      const contentId = inferResolutionContentId(row.image_name);
      const pixelCount = Number(row.working_pixel_count);
      ...
      return {
        ...
        content_category: row.content_category,
        source_format: row.source_format,
        width: row.working_width,
        height: row.working_height,
        pixel_count: pixelCount,
        quantization_level: row.quantization_level,
        quantization_status: row.quantization_status,
        unique_symbol_count: row.unique_symbol_count,
        rle_pair_count: row.rle_pair_count,
        mean_run_length: row.mean_run_length,
        rle_payload_bits: row.rle_payload_bits,
        huffman_payload_bits: row.huffman_payload_bits,
        rle_compression_ratio: row.rle_compression_ratio,
        huffman_compression_ratio: row.huffman_compression_ratio,
        rle_space_saving_percent: row.rle_space_saving_percent,
        huffman_space_saving_percent: row.huffman_space_saving_percent,
        reconstruction_mse: row.reconstruction_mse,
        reconstruction_psnr_db: row.reconstruction_psnr_db,
        rle_encode_mean_ms: row.benchmark_rle_encode_mean_ms,
        rle_decode_mean_ms: row.benchmark_rle_decode_mean_ms,
        rle_total_mean_ms: row.benchmark_rle_total_mean_ms,
        rle_total_min_ms: row.benchmark_rle_total_min_ms,
        rle_total_max_ms: row.benchmark_rle_total_max_ms,
        rle_total_std_ms: row.benchmark_rle_total_std_ms,
        huffman_encode_mean_ms: row.benchmark_huffman_encode_mean_ms,
        huffman_decode_mean_ms: row.benchmark_huffman_decode_mean_ms,
        huffman_total_mean_ms: row.benchmark_huffman_total_mean_ms,
        huffman_total_min_ms: row.benchmark_huffman_total_min_ms,
        huffman_total_max_ms: row.benchmark_huffman_total_max_ms,
        huffman_total_std_ms: row.benchmark_huffman_total_std_ms,
        rle_byte_identical: row.rle_byte_identical,
        huffman_byte_identical: row.huffman_byte_identical,
        rle_payload_bits_per_pixel: pixelCount ? row.rle_payload_bits / pixelCount : "",
        huffman_payload_bits_per_pixel: pixelCount ? row.huffman_payload_bits / pixelCount : "",
        rle_pairs_per_1000_pixels: pixelCount ? row.rle_pair_count / pixelCount * 1000 : "",
        rle_time_ms_per_megapixel: megapixels ? row.benchmark_rle_total_mean_ms / megapixels : "",
        huffman_time_ms_per_megapixel: megapixels ? row.benchmark_huffman_total_mean_ms / megapixels : "",
      };
    });
}

export function summarizeResolutionExperimentRows(rows) {
  const byResolution = new Map();
  for (const row of rows) {
    const key = `${row.width}x${row.height}`;
    if (!byResolution.has(key)) byResolution.set(key, []);
    byResolution.get(key).push(row);
  }
  return [...byResolution.values()].map((items) => {
    const first = items[0];
    const processed = items.filter((row) => row.quantization_status !== "SKIPPED");
    return {
      width: first.width,
      height: first.height,
      ...
      processed_row_count: processed.length,
      skipped_count: items.filter((row) => row.quantization_status === "SKIPPED").length,
      content_id_count: new Set(items.map((row) => row.content_id)).size,
      mean_rle_compression_ratio: mean(processed.map((row) => row.rle_compression_ratio)),
      mean_huffman_compression_ratio: mean(processed.map((row) => row.huffman_compression_ratio)),
      mean_rle_space_saving_percent: mean(processed.map((row) => row.rle_space_saving_percent)),
      mean_huffman_space_saving_percent: mean(processed.map((row) => row.huffman_space_saving_percent)),
      mean_reconstruction_mse: mean(processed.map((row) => row.reconstruction_mse)),
      mean_reconstruction_psnr_db: mean(processed.map((row) => row.reconstruction_psnr_db).filter(Number.isFinite)),
      mean_rle_total_time_ms: mean(processed.map((row) => row.rle_total_mean_ms)),
      mean_huffman_total_time_ms: mean(processed.map((row) => row.huffman_total_mean_ms)),
      mean_rle_time_ms_per_megapixel: mean(processed.map((row) => row.rle_time_ms_per_megapixel)),
      mean_huffman_time_ms_per_megapixel: mean(processed.map((row) => row.huffman_time_ms_per_megapixel)),
    };
  });
}

export function isControlledResolutionExperimentRow(row) {
  const contentId = inferResolutionContentId(row.image_name);
  return row.source_format === "PNG"
    && row.processing_mode === "original"
    && row.was_resized === false
    && Number(row.source_width) === Number(row.working_width)
    && Number(row.source_height) === Number(row.working_height)
    && RESOLUTION_EXPERIMENT_LEVELS.includes(Number(row.quantization_level))
    && RESOLUTION_EXPERIMENT_CONTENT_IDS.includes(contentId)
    && row.benchmark_enabled === true;
}

export function inferResolutionContentId(fileName) {
  const base = String(fileName || "").toLowerCase();
  if (base.includes("logo")) return "logo";
  if (base.includes("jalan")) return "jalan";
  if (base.includes("pola")) return "pola";
  return "unknown";
}

export function validateControlledResolutionExperimentRows(rows) {
  const errors = [];
  const byContent = new Map();
  for (const row of rows) {
    const contentId = inferResolutionContentId(row.image_name);
    if (row.source_format !== "PNG") errors.push(`${row.image_name}: source_format must be PNG`);
    if (row.processing_mode !== "original") errors.push(`${row.image_name}: processing_mode must be original`);
    if (row.was_resized !== false) errors.push(`${row.image_name}: was_resized must be false`);
    if (Number(row.source_width) !== Number(row.working_width) || Number(row.source_height) !== Number(row.working_height)) {
      errors.push(`${row.image_name}: working resolution must equal source resolution`);
    }
    if (!RESOLUTION_EXPERIMENT_CONTENT_IDS.includes(contentId)) errors.push(`${row.image_name}: content_id is not part of controlled experiment`);
    if (!RESOLUTION_EXPERIMENT_LEVELS.includes(Number(row.quantization_level))) errors.push(`${row.image_name}: quantization level is not valid for controlled experiment`);
    if (!byContent.has(contentId)) byContent.set(contentId, []);
    byContent.get(contentId).push(row);
  }

  for (const contentId of RESOLUTION_EXPERIMENT_CONTENT_IDS) {
    const items = byContent.get(contentId) ?? [];
    const resolutionSet = new Set(items.map((row) => `${row.source_width}x${row.source_height}`));
    const aspectSet = new Set(items.map((row) => reduceAspect(Number(row.source_width), Number(row.source_height))));
    if (resolutionSet.size !== RESOLUTION_EXPERIMENT_RESOLUTIONS.length) {
      errors.push(`${contentId}: must contain four controlled resolutions`);
    }
    if (aspectSet.size > 1) {
      errors.push(`${contentId}: aspect ratio must be consistent across resolutions`);
    }
    for (const [width, height] of RESOLUTION_EXPERIMENT_RESOLUTIONS) {
      const levels = items
        .filter((row) => Number(row.source_width) === width && Number(row.source_height) === height)
        .map((row) => Number(row.quantization_level))
        .sort((a, b) => a - b);
      if (levels.join(",") !== [...RESOLUTION_EXPERIMENT_LEVELS].sort((a, b) => a - b).join(",")) {
        errors.push(`${contentId} ${width}x${height}: must contain five valid quantization levels`);
      }
    }
  }
  return {
